...
x1 = X_mat(2, 1); x2 = X_mat(4, 1);
y1 = X_mat(2, 2); y2 = X_mat(4, 2);
z1 = X_mat(2, 3); z2 = X_mat(4, 3);
P1 = [(linspace(x1,x2,n_p1))',(linspace(y1,y2,n_p1))',(linspace(z1,z2,n_p1))'];
robots{1,1} = P1;
robots{1,2} = ones(length(P1),1);
robots{1,2} = 20 * robots{1,2};
...
robots{2,1} = P2; 
robots{2,2} = ones(length(P2),1);
robots{2,2} = 20 * robots{2,2};

x1 = X_mat(7, 1); x2 = X_mat(8, 1);
y1 = X_mat(7, 2); y2 = X_mat(8, 2);
z1 = X_mat(7, 3); z2 = X_mat(8, 3);
P3 = [(linspace(x1,x2,n_p3))',(linspace(y1,y2,n_p3))',(linspace(z1,z2,n_p3))'];
robots{3,1} = P3;
robots{3,2} = ones(length(P3),1);
robots{3,2} = 20 * robots{3,2};
